      ...
      <div class="main-card">
        ...
        <transition name="el-zoom-in-top">
          ...
          <div v-else class="input-panel">
            <div v-if="mode === 'receive'" class="receive-panel">
              <div class="receive-head">
                <h3>输入取件码</h3>
                <p>只需要输入取件码，即可跳转到文件详情界面。</p>
              </div>

              <el-input
                v-model="receiveCode"
                maxlength="12"
                placeholder="例如 E82UDD"
                class="receive-input"
                @keyup.enter="goReceive"
              >
                <template #append>
                  <button class="paste-btn" type="button" @click="pasteAndReceive">
                    粘贴
                  </button>
                </template>
              </el-input>
              ...
              <div class="tab-footer">
                <el-button
                  ...
                  @click="goReceive"
                >
                  <el-icon><Promotion /></el-icon> 立即取件
                </el-button>
              </div>
            </div>
...
import api from '../api'
import { fetchParcelMeta } from '../lib/api'

const router = useRouter()

// 状态
const mode = ref('send')
const theme = ref(localStorage.getItem('pickup-theme') || 'light')
...
const toggleMode = () => {
  mode.value = mode.value === 'send' ? 'receive' : 'send'
}
...
const shareText = async () => {
  ...
  try {
    ...
    formData.append('file', new File([textContent.value], 'shared_text.txt', { type: 'text/plain' }))
    const { data } = await api.post('/file-share/upload', formData)

    handleSuccess(data, {
      filename: '',
      size: new TextEncoder().encode(textContent.value).length
    })
...
const goReceive = () => {
  const openReceive = async () => {
    ...
    try {
      const meta = await fetchParcelMeta(code)
      if (meta.is_expired) {
        ElMessage.error('该取件码已失效')
        return
      }

      router.push(`/pickup/${code}`)
    } catch {
      ElMessage.error('取件码不存在')
    }
  }
...
const pasteAndReceive = async () => {
  try {
    const text = await navigator.clipboard.readText()
    receiveCode.value = text.trim().toUpperCase()
    goReceive()
  } catch {
...
const handleSuccess = (data, extra = {}) => {
  shareInfo.value = {
    ...
    size: extra.size ?? data.size
  }
  shareLink.value = withTheme(
    data.pickup_url || `${window.location.origin}/pickup/${data.code || data.file_id}`
  )
  startCountdown()
  // 生成二维码
  nextTick(() => {
...
const viewShare = (fileId) => {
  const url = `${window.location.origin}/pickup/${fileId}`
...
.app-container > *:not(.grid-background) {
  position: relative;
  z-index: 1;
}
...
@keyframes meteor-fall {
  ...
  100% {
    opacity: 0;
    transform: translate3d(-520px, 320px, 0) rotate(-28deg);
  }
}

.app-container.dark .brand-box,
.app-container.dark .main-card,
.app-container.dark .tips-card-bordered,
.app-container.dark .admin-section,
.app-container.dark .theme-toggle,
.app-container.dark .mode-btn,
.app-container.dark .file-preview-item,
.app-container.dark .info-card-bordered,
.app-container.dark .qr-code-box {
  background: #181b1f;
  border-color: #343a40;
  color: #e5e7eb;
  box-shadow: none;
...
.app-container.dark .success-header h3,
.app-container.dark .receive-head h3,
.app-container.dark .upload-texts h4,
.app-container.dark .section-header h3,
.app-container.dark .share-name,
.app-container.dark .tips-header {
  color: #f9fafb;
}

.app-container.dark .brand-text p,
.app-container.dark .success-header p,
.app-container.dark .receive-head p,
.app-container.dark .upload-texts p,
.app-container.dark .tip-point,
.app-container.dark .share-meta,
.app-container.dark .empty-state p,
...
.app-container.dark .custom-tabs :deep(.el-tabs__header) {
  background: #15181b;
  border-bottom-color: #343a40;
}

.app-container.dark .custom-tabs :deep(.el-tabs__item) {
  color: #9ca3af;
}

.app-container.dark .custom-tabs :deep(.el-tabs__item.is-active) {
  color: #f9fafb;
  background: #181b1f;
}

.app-container.dark .custom-tabs :deep(.el-tabs__active-bar) {
  background-color: #f3f4f6;
}

.app-container.dark .mono-textarea :deep(.el-textarea__inner),
.app-container.dark .receive-input :deep(.el-input__wrapper),
.app-container.dark .link-input :deep(.el-input__wrapper) {
  background: #121417;
  color: #f3f4f6;
  box-shadow: 0 0 0 1px #343a40 inset !important;
}

.app-container.dark .receive-input :deep(.el-input__wrapper.is-focus),
.app-container.dark .mono-textarea :deep(.el-textarea__inner:focus),
.app-container.dark .link-input :deep(.el-input__wrapper.is-focus) {
  box-shadow: 0 0 0 1px #ffffff inset !important;
}

.app-container.dark .receive-input :deep(.el-input__inner)::placeholder,
.app-container.dark .mono-textarea :deep(.el-textarea__inner)::placeholder {
  color: #6b7280;
}

.app-container.dark .paste-btn {
  background: #121417;
  border-color: #343a40;
  color: #f3f4f6;
}

.app-container.dark .receive-panel,
.app-container.dark .input-panel {
  background: #181b1f;
}

.app-container.dark .submit-btn,
.app-container.dark .copy-btn,
.app-container.dark .action-footer .el-button,
.app-container.dark .tab-footer .el-button {
  background: #f3f4f6 !important;
  border-color: #f3f4f6 !important;
  color: #111 !important;
}

.app-container.dark .action-footer .el-button.is-plain,
.app-container.dark .action-footer .el-button:not(.el-button--primary) {
  background: #20242a !important;
  border-color: #343a40 !important;
  color: #f3f4f6 !important;
}

.app-container.dark .border-upload :deep(.el-upload-dragger) {
  background: #121417;
  border-color: #4b5563;
}

.app-container.dark .icon-circle,
.app-container.dark .file-icon {
  background: #111315;
  border-color: #343a40;
  color: #d1d5db;
}

.app-container.dark .qr-code-area {
...
.receive-input :deep(.el-input__wrapper) {
  min-height: 48px;
  border-radius: 10px;
  box-shadow: 0 0 0 1px #d1d5db inset !important;
}

.receive-input :deep(.el-input-group__append) {
  padding: 0;
  border: 0;
  background: transparent;
  box-shadow: none;
...
.receive-input :deep(.el-input__wrapper.is-focus) {
